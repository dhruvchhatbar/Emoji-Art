...
struct EmojiArtDocumentView: View {
    ...
    @State private var pan: CGSize = .init(width: 100, height: 100)
    ...
    private func documentContents(in geometry: GeometryProxy) -> some View{
        AsyncImage(url: document.background){ phase in
            ...
            .position(Emoji.Positionn(x: 0, y: 0).in(geometry))
    ...
    private func emojiPosition(at location: CGPoint, in geometry: GeometryProxy) -> Emoji.Positionn{
        ...
        return Emoji.Positionn(
            x: ((location.x - center.x) - pan.width) / zoom,
            y: -(location.y - center.y - pan.height) / zoom
        )
    }
}
